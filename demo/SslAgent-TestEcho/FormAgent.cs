...
#pragma warning disable IDE0069 // 应释放可释放的字段
        private readonly ISslAgent _agent = new SslAgent();
#pragma warning restore IDE0069 // 应释放可释放的字段
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            _agent.VerifyMode = SslVerifyMode.Peer | SslVerifyMode.FailIfNoPeerCert;
            // 单向验证或客户端可选
            _agent.CaPemCertFileOrPath = "ssl-cert\\ca.crt";
            // 以下都是客户端可选的属性
            _agent.PemCertFile = "ssl-cert\\client.cer";
            _agent.PemKeyFile = "ssl-cert\\client.key";
            _agent.KeyPassword = "123456";

            // 初始化ssl环境, 参数false表示从文件加载证书, 如果为true, 应该把证书读到内存
            // 例如, 证书是数据库里读入进来的文本数据, 此处为true
            if (!_agent.Initialize(false))
            {
            ...
            */
            ...
            // ssl agent 还应该绑定握手事件, 该事件到达说明两端握手成功, OnConnect只能说明连接成功了, 还没握手
            _agent.OnHandShake += OnHandShake;
        ...
        // ReSharper disable once InconsistentNaming
        private HandleResult OnHandShake(IAgent sender, IntPtr connId)
        {
            AddLog($"OnHandShake({connId})");
