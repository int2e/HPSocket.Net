...
#pragma warning disable IDE0069 // 应释放可释放的字段
        private readonly ISslServer _server = new SslServer();
...
#pragma warning restore IDE0069 // 应释放可释放的字段
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            _server.VerifyMode = SslVerifyMode.Peer | SslVerifyMode.FailIfNoPeerCert;
            // 单向验证或客户端可选
            _server.CaPemCertFileOrPath = "ssl-cert\\ca.crt";
            _server.PemCertFile = "ssl-cert\\server.cer";
            _server.PemKeyFile = "ssl-cert\\server.key";
            _server.KeyPassword = "123456";

            // 初始化ssl环境, 参数false表示从文件加载证书, 如果为true, 应该把证书读到内存
            // 例如, 证书是数据库里读入进来的文本数据, 此处为true
            if (!_server.Initialize(false))
            {
            ...
            _server.OnHandShake += OnHandShake;
