...
namespace HPSocket
{
    ...
    public interface IHttpSyncClient : IHttpClient
    {
        ...
        /// <param name="forceReconnect">是否强制重新连接（默认：false，当请求 URL 的主机和端口与现有连接一致时，重用现有连接）</param>
        /// <returns></returns>
        byte[] Get(string url, bool forceReconnect = false);
        ...
        /// <param name="forceReconnect">是否强制重新连接（默认：false，当请求 URL 的主机和端口与现有连接一致时，重用现有连接）</param>
        /// <returns></returns>
        byte[] Post(string url, byte[] body, int length, bool forceReconnect = false);
        ...
        /// <param name="forceReconnect">是否强制重新连接（默认：false，当请求 URL 的主机和端口与现有连接一致时，重用现有连接）</param>
        /// <returns></returns>
        byte[] Put(string url, byte[] body, int length, bool forceReconnect = false);
        ...
        /// <param name="forceReconnect">是否强制重新连接（默认：false，当请求 URL 的主机和端口与现有连接一致时，重用现有连接）</param>
        /// <returns></returns>
        byte[] Delete(string url, bool forceReconnect = false);
